...
**CPU 使用率采用常驻采样（预热）。** 操作系统层面的 CPU 使用率没有"瞬时值"——
`GetSystemTimes` / `/proc/stat` 给的是自开机以来的累计 tick，必须两次采样求差才能算出使用率，
所以它本质上永远是"某个区间的平均值"。若只在 `/api/stats` 被请求时才采样，
离开仪表盘一段时间再切回来，第一次读数会覆盖整段空档（可能是几分钟的平均值）。

因此后台以 `util.CPUSampleInterval`（默认 **1 秒**）常驻采样并缓存，接口只读缓存：
读数始终是"最近 1 秒"的使用率，切回仪表盘第一眼就是最新值，不存在旧值闪烁。
希望数字更平滑时，把该常量改为 `3 * time.Second` 即可。
